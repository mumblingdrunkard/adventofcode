...
fn get_kind(a: &str) -> HandKind {
    ...
    if m.len() == 1 {
        ...
        let v = m.values().copied().collect::<Vec<_>>();
        match v[..] {
            [2, 3] | [3, 2] => match m.contains_key(&'J') {
                true => HandKind::FiveOfAKind,
                false => HandKind::FullHouse,
            },
            [1, 4] | [4, 1] => match m.contains_key(&'J') {
                true => HandKind::FiveOfAKind,
        ...
        let v = m.values().copied().collect::<Vec<_>>();
        match v[..] {
            [3, 1, 1] | [1, 3, 1] | [1, 1, 3] => match m.get(&'J') {
                Some(1 | 3) => HandKind::FourOfAKind,
                _ => HandKind::ThreeOfAKind,
            },
            [1, 2, 2] | [2, 1, 2] | [2, 2, 1] => match m.get(&'J') {
